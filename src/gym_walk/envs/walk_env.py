"""Gymnasium environment for random walk by Miguel Morales for Grokking Deep Reinforcement Learning."""
...
class WalkEnv(gym.Env):
    ...
    def __init__(
        self, n_states=7, p_stay=0.0, p_backward=0.5, render_mode=None, verbose=0
    ):
        ...
        for s in range(nS):
            ...
            for a in range(nA):
                ...
                # Termination checks
                d_forward = (s >= nS - 2 and s_forward == nS - 1) or (
                    s <= 1 and s_forward == 0
                )
                d_backward = (s >= nS - 2 and s_backward == nS - 1) or (
                    s <= 1 and s_backward == 0
                )
                ...
                assert np.isclose(
                    p_forward + p_stay + p_backward, 1.0
    ...
    def render(self):
        ...
        if mode == "ansi":
            ...
            desc = np.asarray(
                ["[" + ascii_uppercase[: self.shape[1] - 2] + "]"], dtype="c"
            ).tolist()
            desc = [[c.decode("utf-8") for c in line] for line in desc]
            color = (
                "red" if self.s == 0 else "green" if self.s == self.nS - 1 else "yellow"
            )
...
if __name__ == "__main__":
    ...
    for s in range(env.nS):
        for a in range(env.nA):
            ...
            assert np.isclose(
                sum(probs), 1.0
            ), f"Probabilities for state {s}, action {a} do not sum to 1"
